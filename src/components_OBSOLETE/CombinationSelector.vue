    ...
    <div class="popup-content">
      ...
      <ul class="space-y-2">
        <li v-for="combo in options" :key="combo.name">
          <button
            @click="choose(combo)"
            class="w-full bg-blue-100 hover:bg-blue-200 rounded p-2 text-left"
          >
...
import type { CardData } from "@/game/types"; // ou où tu stockes tes types
...
<script setup lang="ts">
defineProps<{
  options: Array<{ name: string; points: number; cards: Card[] }>;
}>();
