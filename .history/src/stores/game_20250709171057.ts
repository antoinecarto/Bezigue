...
export const useGameStore = defineStore("game", () => {
  ...
  async function addToMeld(uid: string, code: string) {
    ...
    /* 1. MAJ LOCALE (optimiste & réactive) -------------------------------- */
    hand.value = hand.value.filter((c) => c !== code); // nouveau tableau main

    const updatedMeld = [...(melds.value[uid] ?? []), code]; // nouveau tableau meld
    melds.value = { ...melds.value, [uid]: updatedMeld }; // nouvel objet
    ...
    try {
      await runTransaction(db, async (tx) => {
        ...
        /* sécurité : la carte doit être encore dans la main serveur */
      ...
      /* 3. ROLLBACK LOCAL -------------------------------------------------- */
      console.error(err);
      hand.value = [...hand.value, code]; // remet dans la main
