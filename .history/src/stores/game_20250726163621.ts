  ...
  async function drawCard() {
    ...
    await runTransaction(db, async (tx) => {
      ...
      if (deck.length === 0) {
        ...
        const trump = d.trumpCard;
        const trumpSuit = trump.slice(-1); // ex. ♠
        const trumpRank = trump.slice(0, -1); // ex. "7" ou "J"

        const hadSeven = hand.some((card) => card === "7" + trumpSuit);

        // On ne donne la trumpCard que si elle n'a pas déjà été échangée (i.e. elle n'est pas un 7)
        if (trumpRank !== "7") {
          const finalHand = [...hand, trump];
          update[`hands.${myUid.value}`] = finalHand;
        }
      }
