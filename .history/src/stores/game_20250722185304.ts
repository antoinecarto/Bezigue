...
export const useGameStore = defineStore("game", () => {
  ...
  async function addToMeld(uid: string, code: string) {
    console.log("🎯 addToMeld appelé avec", uid, code);
    console.log("📦 room.value =", room.value);
    console.log("🖐️ hand.value =", hand.value);

    if (!room.value) {
      console.warn("⛔ Pas de room, on quitte addToMeld");
      return;
    }

    if (!hand.value.includes(code)) {
      console.warn(`⛔ La main ne contient pas ${code}, on quitte addToMeld`);
      return;
    }
    if (!room.value) return;
    ...
    try {
      ...
      // ✅ Puis mise à jour locale si succès
      hand.value = newHand;
      melds.value = {
        ...melds.value,
        [uid]: newMeld,
    ...
    console.log("✅ addToMeld envoyé vers Firestore :", newMeld);
  }
