...
export const useGameStore = defineStore("game", () => {
  ...
  async function addToMeld(uid: string, code: string) {
    console.log("room.value.hands =", room.value.hands);
    console.log("room.value.melds =", room.value.melds);
    console.log("uid =", uid);
    if (!room.value) return;
    ...
    console.log("Main du joueur:", currentHand);
    console.log("Code cherché:", code);
    console.log("Présence exacte dans la main ?", currentHand.includes(code));

    // Vérifie que la carte est bien dans la main locale
    if (!currentHand.includes(code)) {
      console.warn(`⛔️ ${code} n'est pas dans la main`);
      return;
    }

    // Validation taille locale
    if (currentHand.length - 1 + currentMeld.length + 1 > 9) {
    ...
    try {
      await updateDoc(doc(db, "rooms", room.value.id), {
        [`hands.${uid}`]: arrayRemove(code),
        [`melds.${uid}`]: arrayUnion(code),
      });
    } catch (e) {
