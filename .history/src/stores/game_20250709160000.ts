...
export const useGameStore = defineStore("game", () => {
  ...
  //
  /* --------------------------------------------------------- */
  /** Ajoute `code` à la meld de `uid`
   *  - tolère la phase "draw" (si le joueur a cliqué « Terminer la pose » trop tôt)
   *  - déclenche la réactivité en recréant *à chaque fois* un nouvel objet/array
   */
  ...
  /**
   * Déplace immédiatement la carte `code` de la main de `uid`
   * vers sa zone de meld, puis synchronise Firestore.
   * – Tolère que la phase soit déjà "draw" (si le joueur a cliqué
   *   « Terminer la pose » trop tôt) ;
   * – Déclenche la réactivité Vue 3 en recréant toujours
   *   nouveaux tableau et objet.
   */
  async function addToMeld(uid: string, code: string) {
    if (
      !room.value ||
      !["meld", "draw"].includes(room.value.phase) // <- tolérance
    ) {
      return;
    }

    /* 1️⃣  MISE À JOUR LOCALE (optimiste & réactive) ------------------ */
    hand.value = hand.value.filter((c) => c !== code); // nouveau tableau

    const updatedMeld = [...(melds.value[uid] ?? []), code]; // nouveau tableau
    melds.value = {
      // nouvel objet
      ...melds.value,
      [uid]: updatedMeld,
    ...
    try {
      await runTransaction(db, async (tx) => {
        ...
        const srvHand = [...(d.hands[uid] ?? [])];
        const idx = srvHand.indexOf(code);
        if (idx === -1) throw new Error("Card already removed");
      ...
      /* 3️⃣  ROLLBACK LOCAL en cas d’échec --------------------------- */
      hand.value = [...hand.value, code]; // remet la carte
      const rollback = (melds.value[uid] ?? []).filter((c) => c !== code);
      melds.value = { ...melds.value, [uid]: rollback };
      console.error(err);
    }
  }
