...
export const useGameStore = defineStore("game", () => {
  ...
  function getMeldTags(uid: string): Record<string, string[]> {
    ...
    const meldCombos = this.combos[uid] ?? [];
  ...
  function getCardPoints(code: string): number {
    const [suit, rank] = code.split("-");
    const uid = myUid.value;
    const meld = melds.value[uid] ?? [];

    if (rank !== "K" && rank !== "Q") return 0;

    const counterpart = `${suit}-${rank === "K" ? "Q" : "K"}`;
    const bothPresent = meld.includes(code) && meld.includes(counterpart);

    return bothPresent ? 20 : 0;
  }
