...
export const useGameStore = defineStore("game", () => {
  ...
  function isCardPlayableInBattle(
    ...
    if (hasLeadSuit) {
      if (cardInfo.suit === leadSuit) {
        ...
        return {
          playable: false,
          reason: `Vous devez jouer  ${suitLetterToSymbol(
            suitLabel(leadSuit)
    ...
    if (hasTrump) {
      if (cardInfo.suit === trumpSuit) {
        ...
        return {
          playable: false,
          reason: `Vous n'avez pas de  ${suitLetterToSymbol(
            suitLabel(leadSuit)
    ...
    console.log(
      "rerereeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee : ",
      suitLetterToSymbol(suitLabel(leadSuit))
    );
  ...
  function suitLetterToSymbol(letter: string): Suit {
    switch (letter) {
      case "H":
        return "♥";
      case "D":
