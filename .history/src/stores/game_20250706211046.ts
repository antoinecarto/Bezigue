...
export const useGameStore = defineStore("game", () => {
  ...
  async function playCard(code: string) {
    ...
    await runTransaction(db, async (tx) => {
      ...
      /* 2. sécurité : pli déjà complet ? */
      if ((data.trick.cards?.length ?? 0) >= 2) {
        throw new Error("Trick already full");
      }

      /* 3. main serveur : cherche la carte quel que soit _1/_2 */
      ...
      const serverCard = handSrv[idxSrv]; // carte exacte avec suffixe
      handSrv.splice(idxSrv, 1); // retire de la main

      /* 4. ajoute la carte au pli courant */
      ...
      /* 5. si 2 cartes -> déterminer le vainqueur et passer la main */
      if (cards.length === 2) {
        const leadSuit = splitCode(cards[0]).suit;
        const winner =
          compareCards(cards[0], cards[1], data.trumpSuit as Suit, leadSuit) >=
          0
            ? players[0]
            : players[1];

        update.currentTurn = winner; // la main va au vainqueur
        update.trick = { cards: [], players: [] }; // on vide le pli
        // TODO : comptabiliser le pli / score si besoin
      }
