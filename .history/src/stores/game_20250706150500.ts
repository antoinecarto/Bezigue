...
export const useGameStore = defineStore("game", () => {
  ...
  async function playCard(code: string, uid: string) {
    if (!room.value || room.value.phase !== "play") return;

    await runTransaction(db, async (tx) => {
      const roomRef = doc(db, "rooms", room.value.id); // il faut bien le chemin complet
      const roomSnap = await tx.get(roomRef);
      if (!roomSnap.exists()) throw new Error("Room does not exist");

      const roomData = roomSnap.data() as RoomDoc;

      // Retirer la carte de la main côté serveur
      const playerHand = [...(roomData.hands?.[uid] || [])];
      const idx = playerHand.indexOf(code);
      if (idx === -1) throw new Error("Card not found in hand");
      playerHand.splice(idx, 1);

      // Mettre à jour exchangeTable côté serveur
