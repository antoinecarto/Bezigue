...
export const useGameStore = defineStore("game", () => {
  ...
  async function resolveTrickOnServer(): Promise<void> {
    ...
    await runTransaction(db, async (tx) => {
      ...
      // Vérifier si après ce pli, il n'y aura plus de cartes à jouer
      const allHandsWillBeEmpty = d.players.every((uid) => {
        const handData = d.hands?.[uid];
        const handSize = Array.isArray(handData) ? handData.length : 0;
        // Après avoir joué ce pli, chaque main aura une carte en moins
        return handSize <= 1; // Si ≤1, après le pli ce sera 0
      });

      const allMeldsWillBeEmpty = d.players.every((uid) => {
      ...
      // C'est le dernier pli si après ce pli tout sera vide ET qu'on ne peut plus piocher
      const isLastTrick =
        allHandsWillBeEmpty && allMeldsWillBeEmpty && deckEmpty;
