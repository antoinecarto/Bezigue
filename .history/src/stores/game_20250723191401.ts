...
export const useGameStore = defineStore("game", () => {
  ...
  async function playCard(code: string) {
    ...
    // ✅ Calcul du total de cartes distribuées (dans les mains)
    const allHands = Object.values(room.value.hands ?? {});
    const totalCards = allHands.reduce((sum, hand) => sum + hand.length, 0);

    const isFirstTurn = totalCards < 18;
    // 🚫 Bloque si l'adversaire n’a pas encore pioché
