...
import { Card, Suit, Rank } from "@/game/types/Card";
...
export default class BezigueGame {
  ...
  isGameOver(): boolean {
    return (
      this.player1Hand.length === 0 &&
      this.player2Hand.length === 0 &&
  ...
  getPossibleCombinations(cards: Card[]): Combination[] {
    ...
    const isCardUsedIn = (card: Card, usedSet: Set<string>) =>
      usedSet.has(getCardKey(card));
    ...
    for (const rank of ranksToCheck) {
      const available = countByRank[rank].filter(
        (c) => !isCardUsedIn(c, usedCardsInQuads)
      );
    ...
    for (const suit of ["♠", "♥", "♦", "♣"] as Suit[]) {
      const kings = cards.filter(
        (c) =>
          c.rank === "K" &&
          c.suit === suit &&
          !isCardUsedIn(c, usedCardsInMarriages)
      );
      const queens = cards.filter(
        (c) =>
          c.rank === "Q" &&
          c.suit === suit &&
          !isCardUsedIn(c, usedCardsInMarriages)
      ...
      const suiteCards = suiteRanks.map((rank) =>
        cards.find(
          (c) =>
            c.suit === suit &&
            c.rank === rank &&
            !isCardUsedIn(c, usedCardsInSuites)
        )
      );
    ...
    const queensSpades = cards.filter(
      (c) =>
        c.suit === "♠" &&
        c.rank === "Q" &&
        !isCardUsedIn(c, usedCardsInDameValet)
    );
    const jacksDiamonds = cards.filter(
      (c) =>
        c.suit === "♦" &&
        c.rank === "J" &&
        !isCardUsedIn(c, usedCardsInDameValet)
