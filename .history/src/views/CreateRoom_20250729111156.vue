...
async function actuallyCreateRoom() {
  ...
  try {
    ...
    // S’assurer que la trumpCard est à la fin du deck
    const filteredDrawPile = distrib.drawPile.filter(
      (c) => c !== distrib.trumpCard
    );
    filteredDrawPile.push(distrib.trumpCard);
    /* 4. document « rooms » */
