...
const myHand = computed<string[]>({
  get() {
    if (!hand.value || !myUid.value) return [];
    const raw = hand.value[myUid.value];
    if (!raw) return [];
    if (Array.isArray(raw[0])) {
      // Si le premier élément est un tableau, aplatis toute la structure
      return raw.flat();
    }
    return raw;
  },
  set(newHand: string[]) {
    if (hand.value && myUid.value) {
...
console.log("🧩 Contenu de myHand:", myHand.value);
</script>
