...
/* ① — les refs du store --------------------------------------------- */
const { myUid, room, loading, melds } = storeToRefs(game);
...
const isHost = computed(() => {
  ...
  // Le premier joueur dans la liste est considéré comme l'hôte
  if (room.value.players && room.value.players.length > 0) {
    return room.value.players[0] === myUid.value;
  }

  // personne n'est hôte si on ne peut pas déterminer
  return false;
});
