<template>
  <!-- Écran d’attente tant qu’il n’y a pas deux joueurs -->
  <div v-if="room?.phase === 'waiting'" class="text-center mt-8">
    <p class="text-xl">En attente dʼun adversaire…</p>
    <p class="text-sm text-gray-500">
      Partagez ce lien pour démarrer&nbsp;: <br />
      <code class="text-xs select-all">{{ $route.fullPath }}</code>
    </p>
    <!-- ★ modifié -->
  </div>

  <!-- Modal des combinaisons -->
  <teleport to="body">
    <ComboModal
      v-if="showComboPopup"
  ...
  <div
    class="game-room text-center p-4"
    :class="{ 'pointer-events-none opacity-50': !roomReady }"
  >
    <!-- ★ modifié -->
    ...
    <!-- … (scoreboards et zone d’échange inchangés) … -->

    <!-- MAIN DU JOUEUR -->
    <div v-if="localHand.length" class="player-hand mt-8">
      <!-- Zone de dépôt joueur (melds) -->
      <div
...
function maybeStartGame(tx: Transaction, d: RoomDoc) {
  ...
  tx.update(roomRef, {
    ...
    trumpTaken: false,
    trumpSuit: distrib.trumpCard.slice(-1) as Suit,
    hands,
    melds: {},
    trick: { cards: [], players: [] },
    drawQueue: [],
...
async function playCombo(combo: Combination) {
  ...
  await runTransaction(db, async (tx) => {
    ...
    /* 3. Contrôle double‑paquet ---------------------------------- */
    if (!checkDoubleDeckLimit(hand, melds)) {
      throw "Impossible : plus de deux exemplaires identiques.";
    }
