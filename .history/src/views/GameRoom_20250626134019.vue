...
/** Joue une carte déjà posée dans les combinaisons */
async function playCardFromMeld(card: Card) {
  /* mêmes sécurités que playCard -------- */
  ...
  await runTransaction(db, async tx => {
    ...
    const d = snap.data()
    if (!d) throw 'Room manquante'
    if (d.currentTurn !== uid.value) throw 'Pas votre tour'
    if (d.canMeld === uid.value) throw 'Pose de combinaison encore possible ; pioche d’abord'

    /* 1️⃣ retirer la carte du meld ----------------------------------- */
    const melds: Combination[] = d.melds?.[uid.value] ?? []
    let removed = false
    const newMelds = melds.map(m => {
      if (removed) return m                       // déjà trouvé
      const idx = m.cards.findIndex(cc => cc.rank===card.rank && cc.suit===card.suit)
      if (idx !== -1) {
        removed = true
        const cards = [...m.cards]
        cards.splice(idx,1)                       // enlève la carte
        return { ...m, cards }
      }
      return m
    }).filter(m => m.cards.length)                // supprime un meld vide

    if (!removed) throw 'Carte non trouvée dans les combinaisons'
    ...
    /* 3️⃣ si on atteint 2 cartes -> on réutilise la même logique
          que dans playCard pour résoudre le pli */
    if (trick.cards.length === 2) {
      ...
      /* scoring + pioche idem (réutilise le code de playCard) */
      // ... (copie-colle la partie résolution déjà écrite) …

      update.trick       = { cards:[], players:[] }
      update.currentTurn = winner
      update.canMeld     = winner              // nouveau vainqueur peut poser
      // n’oublie pas update.deck + hands si tu appelles la pioche
    } else {
