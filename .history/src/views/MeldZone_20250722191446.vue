...
function onCardDropped(evt: any) {
  ...
  if (toMeld && !fromMeld) {
    // ✅ Carte vient de la main vers le meld
    console.log("➡️ Ajout dans le meld :", addedCard);
    game.addToMeld(props.uid, addedCard);
  } else if (fromMeld && !toMeld) {
    // ⬅️ Carte revient du meld vers la main
    console.log("⬅️ Retrait du meld :", addedCard);
