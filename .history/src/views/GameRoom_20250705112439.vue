...
watchEffect(async () => {
  ...
  /* On attend la fin de la dernière pioche */
  if (r.phase !== "draw" || r.deck.length > 0) return;

  /* Les mains sont‑elles déjà revenues à ≥ 9 ? → on a déjà rapatrié */
  const alreadyMerged = r.players.every(
    (uid) => (r.melds[uid]?.length ?? 0) === 0
  );
  if (alreadyMerged) return; //  ⬅️  stop : on l’a déjà fait
  ...
  await runTransaction(db, async (tx) => {
    ...
    d.players.forEach((uid) => {
      /* 1. remélange les cartes */
      const merged = mergeMeldsIntoHand(d, uid); // renvoie déjà normalisé
      update[`hands.${uid}`] = merged;
      update[`melds.${uid}`] = []; // melds vidés
    });

    /* 2. on relance la bataille  */
    update.phase = "play";
    update.trick = { cards: [], players: [] }; // ⬅️  on vide la zone
    update.canMeld = null;
    update.drawQueue = [];
...
function mergeMeldsIntoHand(d: RoomDoc, uid: string): string[] {
  const current = [...d.hands[uid]]; // main actuelle
  const toAdd = (d.melds[uid] ?? []).flatMap((m) => m.cards.map(cardToStr));

  const merged = [...current, ...toAdd];

  /* 1. jamais plus de 2 exemplaires identiques            */
  /* 2. maximum 9 cartes après rapatriement                */
