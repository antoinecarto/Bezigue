<template>
  ...
  >
    <div class="bg-white rounded-2xl p-6 w-96 text-center shadow-2xl">
      ...
      <button class="btn w-full" @click="tryPlayCard(cardToStr(c))">OK</button>
    </div>
  </div>
...
function handleCardClick(card: string) {
  playCard(card); // ta logique directe
  tryPlayCard(card); // ton wrapper avec gestion d'erreurs / popup
}
