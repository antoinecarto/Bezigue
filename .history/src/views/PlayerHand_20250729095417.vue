...
export const useGameStore = defineStore("game", () => {
  ...
  async function addToMeld(uid: string, code: string): Promise<void> {
    ...
    if (!currentHand.includes(code)) {
      console.warn(`❌ La carte ${code} n'est pas dans la main du joueur ${uid}.`);
      return;
    }
  ...
  async function removeFromMeldAndReturnToHand(uid: string, code: string): Promise<void> {
    if (!room.value || !uid || !code) return;
    ...
    try {
      ...
      console.log(`✔️ Carte ${code} retirée du meld et ajoutée à la main de ${uid}.`);
    } catch (e) {
  ...
  async function doExchangeProcess(): Promise<void> {
    try {
      ...
      if (!newTrumpCard || !room.value) {
        console.warn("Aucune nouvelle carte d'atout, mise à jour du deck annulée.");
        return;
      }
  ...
  async function updateDeckAfterExchange(roomId: string, newTrumpCard: string): Promise<void> {
