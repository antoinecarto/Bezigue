...
async function playCard(card: string) {
  ...
  // même logique que précédemment (non modifiée ici pour la brièveté)
}
...
// État Firestore principal (simplifié)
interface RoomDoc {
  players: string[];
  playerNames: Record<string, string>;
  phase: "play" | "draw" | "meld";
...
function subscribeRoom() {
  return onSnapshot(roomRef, (snap) => {
    if (!snap.exists()) return (room.value = null);
    room.value = snap.data() as RoomDoc;
    loading.value = false;
  });
}
...
/* TODO: refactor playCombo / playCombination de la même façon →
   - plus besoin de passer l'uid
   - vérifie d.canMeld === myUid.value etc.                      */

/* ────────────── Expose to template ───────────────────── */
// Vous pourrez maintenant appeler :
// <Card @click="isMyTurn && playCard(card)" :disabled="!isMyTurn" />
...
async function playCard(uid: string, card: string) {
  await runTransaction(db, async (tx) => {
    const d = (await tx.get(roomRef)).data()!;
    if (d.phase !== "play") throw "Phase play requise";
    if (d.currentTurn !== uid) throw "Pas votre tour";

    // 1. retirer la carte de la main
    const hand = d.hands[uid].filter((c: string) => c !== card);

    // 2. ajouter au trick
    const trick = d.trick ?? { cards: [], players: [] };
    if (trick.cards.length >= 2) throw "Pli non vidé";
    trick.cards.push(card);
    trick.players.push(uid);
    ...
    /* pli complet ? -> passer en phase DRAW */
    if (trick.cards.length === 2) {
      ...
      const loser = trick.players.find((p) => p !== winner)!;
      update.phase = "draw";
      update.drawQueue = [winner, loser]; // ordre des pioches
      update.currentTurn = winner; // jouera le pli suivant
      update.canMeld = null; // on bloquera les melds
      // bonus 10/As …
    } else {
      update.currentTurn = opponentOf(uid, d);
    }
    tx.update(roomRef, update);
  });
}
...
async function playCombo(uid: string, combo: Combination) {
  await runTransaction(db, async (tx) => {
    const d = (await tx.get(roomRef)).data()!;
    if (d.phase !== "meld" || d.canMeld !== uid) throw "Pas la phase de meld";
    ...
    /* 3. reste-t-il des combos ? */
    const hasMore =
      detectCombinations(
        [...hand.map(strToCard), ...melds.flatMap((m) => m.cards)],
        d.trumpCard.slice(-1) as Suit,
        melds
      ).length > 0;
    ...
    const update: any = {
      [`hands.${uid}`]: hand,
      [`melds.${uid}`]: melds,
      canMeld: hasMore ? uid : null,
      phase: hasMore ? "meld" : "draw", // le joueur RE-pioche
      drawQueue: hasMore ? d.drawQueue : [uid],
    };
...
/*────────────────────────────────────────────────────────────────────*/
async function drawCard(uid: string) {
  await runTransaction(db, async (tx) => {
    const d = (await tx.get(roomRef)).data()!;
    if (d.phase !== "draw") throw "Pas la phase de pioche";
    if (d.drawQueue[0] !== uid) throw "Pas votre tour de piocher";

    const deck = [...d.deck];
    if (!deck.length) return; // pioche vide -> fin imminente
    const card = deck.shift()!;

    const hand = [...d.hands[uid], card];
    const queue = d.drawQueue.slice(1); // retire le premier
    ...
    if (queue.length === 0) {
      update.phase = "meld";
      update.canMeld = d.drawQueue[0]; // le vainqueur du pli
      update.trick = { cards: [], players: [] }; // on vide la table
    }
...
/** Pioche 1 carte vainqueur, 1 carte perdant dès la fin de la phase meld */script
