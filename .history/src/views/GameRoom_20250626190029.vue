...
/* ──────── helpers ──────────────────────────────────────────────── */
const cardToStr = (c: Card | string) => typeof c === 'string' ? c : `${c.rank}${c.suit}`;
const strToCard = (s: string): Card => ({ rank: s.slice(0, -1) as Rank, suit: s.slice(-1) as Suit });
...
const playerMelds        = computed<Combination[]>(() => roomData.value?.melds?.[uid.value ?? ''] ?? []);
const playerMeldCards    = computed<Card[]>(() => playerMelds.value.flatMap(c => c.cards));
...
function subscribeMene (roomId: string, meneId: number) {
  return onSnapshot(
    ...
    snap => { battleZoneCards.value = snap.exists() ? snap.data().currentPliCards ?? [] : []; }
  );
}
...
onMounted(() => {
  ...
  watch(currentMeneId, id => {
    if (unsubscribeMene) { unsubscribeMene(); unsubscribeMene = null; }
  ...
  watch(() => roomData.value?.hands?.[uid.value] ?? [], h => { localHand.value = [...h]; }, { immediate: true });

  /* watcher maître : popup, combos, pioche */
  watch(
    () => ({ right: canMeld.value, popup: showComboPopup.value }),
    ({ right, popup }, prev) => {
      /* 1. fermetures si plus le droit de meld */
      if (right !== uid.value && popup) showComboPopup.value = false;

      /* 2. recalcul combos si c’est mon tour de meld */
      if (right === uid.value) {
        const all = [...localHand.value.map(strToCard), ...playerMeldCards.value];
        validCombos.value = detectCombinations(all, trumpCard.value.slice(-1) as Suit, playerMelds.value);
        showComboPopup.value = validCombosFiltered.value.length > 0;
      }

      /* 3. si canMeld devient null (ou popup se ferme) → pioche */
      if ((prev?.right === uid.value && right == null) ||
          (prev?.popup && !popup && canMeld.value == null)) {
        setTimeout(() => drawCardsAfterMeld().catch(console.error), 1000);
      }
    },
    { immediate: true }
  );
...
onUnmounted(() => { if (unsubscribeMene) unsubscribeMene(); });
...
async function playCard(card: string) {
  ...
  await runTransaction(db, async tx => {
    ...
    if (trick.cards.length === 2) {
      ...
      update.canMeld     = winnerUid;          // ouvrira la popup
...
async function playCardFromMeld(card: Card) {
  ...
  await runTransaction(db, async tx => {
    ...
    /* retire la carte du meld */
    const melds: Combination[] = d.melds?.[uid.value] ?? [];
    let   removed = false;
    const newMelds = melds
      .map(m => {
        if (removed) return m;
        const i = m.cards.findIndex(c => c.rank === card.rank && c.suit === card.suit);
        if (i !== -1) { removed = true; const cs=[...m.cards]; cs.splice(i,1); return { ...m, cards: cs }; }
        return m;
      })
    ...
    const meneSnap = await tx.get(meneRef);
    const meneData = meneSnap.exists() ? meneSnap.data() : {};
    tx.set(meneRef, { currentPliCards: [...(meneData.currentPliCards ?? []), cardStr] }, { merge: true });

    /* update Firestore principal */
    const update: any = {
    ...
    if (trick.cards.length === 2) {
      ...
      /* pioche immédiate (phase hors-meld) */
      const deck  = d.deck ?? [];
      const hands = d.hands;
...
async function playCombination(combo: Combination) {
  ...
  await runTransaction(db, async tx => {
    ...
    if (d.canMeld !== uid.value) throw 'Phase meld terminée';
    const okCards = combo.cards.every(c =>
      d.hands[uid.value].includes(cardToStr(c)) ||
      (d.melds?.[uid.value] ?? []).flatMap((m:any)=>m.cards).some((cc:Card)=>cc.rank===c.rank&&cc.suit===c.suit)
    );
    if (!okCards) throw 'Cartes manquantes';

    const newHand = d.hands[uid.value].filter((s:string) =>
      !combo.cards.some(c => s === cardToStr(c)));
    ...
    tx.update(roomRef, {
      ...
      scores,
      canMeld: null               // → débloquera drawCardsAfterMeld
    });
  });
...
/**
 * Pioche automatique appelée 1 s après que canMeld repasse à null
 * (ou après la fermeture de la popup sans meld).
 */
async function drawCardsAfterMeld() {
  ...
  await runTransaction(db, async tx => {
    ...
    if (d.canMeld != null) throw 'Phase meld non terminée';

    const winnerUid = uid.value;
    const loserUid  = Object.keys(d.hands).find(k => k !== winnerUid)!;

    const deck  = [...(d.deck ?? [])];
    const hands = { ...d.hands };

    const meldSize = (ms:any[]|undefined) => ms?.reduce((n,m)=>n+(m.cards?.length??0),0) ?? 0;
...
async function tryExchangeSeven(playerUid: string) {
  await runTransaction(db, async tx => {
    ...
    const { newHand, newTrumpCard, exchanged } =
      exchangeSevenTrump(handArr, trumpSuit, trumpCardCur);
...
const validCombosFiltered = computed(() => {
  ...
  const playedCombos = (roomData.value.melds?.[uid.value] ?? []).map(c => c.name);

  // On filtre validCombos pour enlever celles déjà posées
  return validCombos.value.filter(combo => !playedCombos.includes(combo.name));
});
