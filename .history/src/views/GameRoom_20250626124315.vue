...
/* watch : recalcule à chaque changement de main ou melds */
  watch(
    ...
    () => {
      const all = [...localHand.value.map(strToCard), ...playerMeldCards.value]  // toutes mes cartes dispos
      validCombos.value = detectCombinations(all, trumpCard.value.slice(-1) as Suit)
