        ...
        >
          ...
          >
            <CardSvgSprite
              v-for="c in meld.cards"
              :key="c.rank + c.suit"
              :nomFr="toFrenchName(cardToStr(c))"
              :width="60"
              :height="90"
...
import CardSvgSprite from "@/components/CardSvgSprite.vue";
import { toFrenchName } from "@/utils/toFrenchName";

const localHandFr = computed(() => localHand.value.map(toFrenchName));
const opponentHandFr = computed(() => opponentHand.value.map(() => "back")); // dos
const battleZoneFr = computed(() => battleZoneCards.value.map(toFrenchName));
