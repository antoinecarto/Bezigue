...
const playerMelds: Combination[] = [] // ou avec un initialiseur réel
...
onMounted(() => {
  ...
  watch(
    () => [localHand.value, playerMelds.value],
    () => {
      const allCards = [...localHand.value, ...playerMelds.value];
      validCombos.value = detectCombinations(allCards, trumpCard.value.slice(-1) as Suit);
