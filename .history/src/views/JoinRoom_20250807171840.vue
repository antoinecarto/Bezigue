<template>
  <div text-black dark:text-black>
...
async function fetchRooms() {
  ...
  try {
    // ✅ CORRECTION: Utiliser "status" au lieu de "phase"
    const q = query(collection(db, "rooms"), where("status", "==", "waiting"));
    const qs = await getDocs(q);
    rooms.value = qs.docs.map((d) => ({ id: d.id, ...d.data() }));
    console.log("✅ Rooms récupérées:", rooms.value.length);
  } catch (e) {
    console.error("❌ Erreur fetchRooms:", e);
...
async function maybeStartGame(tx: any, roomRef: any, roomData: any) {
  ...
  tx.update(roomRef, {
    status: "play", // ✅ CORRECTION: "status" au lieu de "phase"
    currentTurn: host,
    trumpTaken: false,
    deck,
    hands,
