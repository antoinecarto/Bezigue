...
/** Vérifie (a) double‑paquet, (b) total ≤ 9. */
function checkHandAndMeld(hand: string[], melds: Combination[]): void {
  // 1) pas plus de 2 copies de la même carte ........................
  const count: Record<string, number> = {};
  for (const c of hand) count[c] = (count[c] ?? 0) + 1;
  for (const m of melds)
    for (const c of m.cards.map(cardToStr)) count[c] = (count[c] ?? 0) + 1;

  if (Object.values(count).some((n) => n > 2))
    throw "Règle : jamais plus de deux exemplaires identiques.";

  // 2) total main + melds ≤ 9 ......................................
  const total = hand.length + melds.reduce((s, m) => s + m.cards.length, 0);
  if (total > 9)
