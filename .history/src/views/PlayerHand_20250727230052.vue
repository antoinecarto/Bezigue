...
const { myUid, hand, drawQueue, currentTurn, room, myHand } = storeToRefs(game);
...
const localHand = ref([...myHand.value]);

watch(myHand, (newVal) => {
  localHand.value = [...newVal];
});

// Et quand une carte est jouée :
game.updateHand(localHand.value);
...
function onCardDroppedBackToHand(evt: any) {
  ...
  if (!myUid.value) {
    console.warn("Utilisateur non connecté, UID manquant.");
    return;
  }
