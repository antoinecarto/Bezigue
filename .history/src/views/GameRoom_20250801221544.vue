...
// 🔧 Modifier le watcher pour utiliser l'attribution atomique
watch(
  () => ({
  ...
  (newState) => {
    ...
    if (isLastTrick && !lastTrickBonusWinner.value) {
      ...
      // 🎯 UTILISER L'ATTRIBUTION ATOMIQUE
      awardLastTrickBonusAtomic(winnerId);
...
async function awardLastTrickBonusAtomic(winnerId: string) {
  console.log("💰 Attribution atomique +10 pts pour", winnerId);
  ...
  try {
    ...
    console.log("✅ +10 pts bonus (atomique) sauvegardés pour", winnerId);
  } catch (error) {
    console.error("❌ Erreur sauvegarde atomique:", error);
  }
}
watch(
