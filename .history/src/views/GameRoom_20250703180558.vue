  ...
  <div class="game-room text-center p-4">
    ...
    <div v-if="opponentHand.length" class="player-hand mt-6">
      ...
      <div class="cards flex gap-2 justify-center flex-wrap">
        <Card :code="'back'" :width="60" :height="90" />
      </div>
      ...
      <div class="drop-zone mt-4">
        ...
        >
          <p>
            Total cartes adverses :
            {{
              opponentMelds.reduce((acc, meld) => acc + meld.cards.length, 0)
            }}
          </p>
          <!-- <template v-for="(meld, i) in opponentMelds" :key="'meld-' + i">

            <Card
              v-for="(card, j) in meld.cards"
              :key="'oppback-' + i + '-' + j"
              code="back"
              :width="60"
              :height="90"
