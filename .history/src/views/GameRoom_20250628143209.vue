...
/*  RESET automatique au début de chaque nouveau pli */
watchEffect(() => {
...
/* --------------------------------------------------------- */
/*  WATCHER 1 : ouverture de la popup « 7 d’atout »          */
/* --------------------------------------------------------- */
watchEffect(() => {
  const r = room.value;
  if (!r || !myUid.value) return;

  if (showExchange.value || asked7ThisTrick.value) return; // déjà vue

  /* Conditions d’ouverture du 7 d’atout */
  ...
  const myTurnMeld = r.phase === "meld" && r.canMeld === myUid.value;

  if (deckOk && cardOk && have7 && myTurnMeld) {
...
/* --------------------------------------------------------- */
/*  WATCHER 2 : ouverture de la popup « combinaisons »       */
/* --------------------------------------------------------- */
watchEffect(() => {
  const r = room.value;
  if (!r || !myUid.value) return;

  /* déjà ouverte ou déjà demandée -> rien */
  if (showComboPopup.value || askedCombiThisTrick.value) return;

  /* on n’ouvre la popup qu’en phase meld, quand c’est mon tour */
  if (!(r.phase === "meld" && r.canMeld === myUid.value)) return;

  /* si la popup 7 est ouverte, ou l’échange n’est pas encore fait, on attend */
  ...
  /* Utilisation directe de ta fonction detectCombinations */
  ...
  if (combos.length) {
    ...
    /* aucune combinaison possible → on passe direct à la pioche */
    forceEndMeldPhase(); // ta fonction qui met phase="draw"
    askedCombiThisTrick.value = true; // pour ne pas boucler
  }
});
...
async function drawCard() {
  ...
  await runTransaction(db, async (tx) => {
    ...
    /* — 1. qui était en tête ? — */
    const leadUid = d.drawQueue[0]; // vainqueur du pli

    /* — 2. pioche normale — */
    const deck = [...d.deck];
    const card = deck.shift()!;
    const hand = [...d.hands[myUid.value], card];
    const queue = d.drawQueue.slice(1); // on enlève MON uid

    /* — 3. distribution éventuelle de la trumpCard — */
    ...
    if (deck.length === 0) update.trumpCard = ""; // visuel retiré

    /* — 5. fin de file ⇒ retour à play, vainqueur garde la main — */
    if (queue.length === 0) {
      update.phase = "play";
      update.currentTurn = leadUid; // ⬅️  main rendue au vainqueur
      update.canMeld = null;
