...
async function validateCards() {
  ...
  /* 1️⃣  on fige la liste pour éviter les doubles appels */
  const toValidate = [...pending.value];
  pending.value = []; // vide la zone verte tout de suite
  // (les cartes restent via `meld`)

  /* 2️⃣  on envoie chaque carte séquentiellement            */
