...
// Améliorations clés pour éviter les conflits avec les données de jeu
...
async function handleCaller() {
  try {
    ...
    // ✅ CORRECTION: Utiliser merge et ne toucher QUE voiceChat
    await updateDoc(roomRef, {
      "voiceChat.offer": {
        type: offer.type,
        sdp: offer.sdp,
      },
      "voiceChat.createdAt": new Date(),
      "voiceChat.status": "waiting",
      "voiceChat.caller": true,
    });
  ...
  try {
    ...
    while (retries < maxRetries) {
      ...
      if (roomSnapshot.exists() && roomData?.voiceChat?.offer) {
        ...
        // ✅ CORRECTION: Mise à jour ciblée, sans affecter les autres données
        await updateDoc(roomRef, {
          "voiceChat.answer": {
            type: answer.type,
            sdp: answer.sdp,
...
// ✅ CORRECTION: Cleanup plus sûr
async function cleanup() {
  isConnected.value = false;
  connectionStatus.value = "";

  // Arrêter le stream local en premier
  if (localStream) {
    localStream.getTracks().forEach((track) => {
      track.stop();
    });
  ...
  // Fermer la connexion peer de façon sécurisée
  if (peerConnection) {
    // Arrêter d'écouter les événements avant de fermer
  ...
  // ✅ Désabonner les listeners Firebase de façon sécurisée
  try {
    if (roomListener) {
      roomListener();
      roomListener = null;
    ...
    console.warn("Erreur lors du nettoyage des listeners:", e);
  }

  // ✅ NE PAS nettoyer les données de la room dans Firestore
  // Car cela pourrait affecter les données de jeu
}
